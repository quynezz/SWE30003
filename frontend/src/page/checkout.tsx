...
export function CheckoutPage() {
    ...
    if (!product) {
        return (
            ...
            <h1 className="text-2xl font-bold text-red-600">Sản Phẩm Không Tồn Tại</h1>
            <p className="text-gray-600">Sản phẩm bạn đang cố gắng mua không tồn tại.</p>
            <Button
            variant="outline"
            className="mt-4"
            onClick={() => navigate("/")}
    ...
    const formattedDiscountAmount = Math.ceil(discountAmount).toString().concat(".000đ");
    const formattedPromoAmount = promoAmount.toString().concat(".000đ");
    const formattedShippingCost = Math.ceil(shippingCost).toString().concat(".000đ");
    const formattedTotalPrice = totalPrice.toString().concat(".000đ")
    ...
    const handleCheckout = () => {
        ...
        setIsLoading(true);
            toast.current?.show({
                severity: 'success',
                summary: 'success',
                detail: 'Thanks for your order! Redirecting to home page...',
                life: 3000,
            });
        setTimeout(() => {
            navigate("/");
